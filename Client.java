...
public class Client {
	...
	private JTextField textField;// 用于编辑显示文本信息
	private JTextField txt_port;// 设置端口号
	private JTextField txt_hostIp;// 设置IP
	private JTextField txt_name;// 设置用户名
	private JButton btn_start;// 开始按钮
	private JButton btn_stop;// 断开按钮
	private JButton btn_send;// 发送按钮
	private JPanel northPanel;// 上面板
	private JPanel southPanel;// 下面板
	private JScrollPane leftScroll;// 左滚动条
	private JScrollPane rightScroll;// 右滚动条
	private JSplitPane centerSplit;// 分割线

	private DefaultListModel<String> listModel;// 默认的列表
	private boolean isConnected = false;// 用来记录客户端连接跟断开

	private Socket socket;
	private PrintWriter writer;// 输出字符打印
	private BufferedReader reader;// 缓存读取
	private MessageThread messageThread;// 负责接收消息的线程
	private Map<String, User> onLineUsers = new HashMap<String, User>();// 所有在线用户
	private static final String TIP = "请输入姓名";// 提示
	...
	// 执行发送
	public void send() {
		String message = textField.getText().trim();// 去掉首尾空格获取文本框内容
		sendMessage(frame.getTitle() + "@" + "ALL" + "@" + message);// 传输用户名及输入的文本信息
		textField.setText(null);// 清空输入文本框内容
	}
	...
	public Client() {
		...
		txt_name = new JTextField(TIP);
		txt_name.setName("txt_name");
		txt_name.setForeground(Color.gray);
		btn_start = new JButton("连接");
		btn_stop = new JButton("断开");
		btn_send = new JButton("发送(s)");
		listModel = new DefaultListModel<String>();
		userList = new JList<String>(listModel);
		btn_start.setFont(new Font("宋体", Font.BOLD, 12));
		btn_stop.setFont(new Font("宋体", Font.BOLD, 12));
		btn_send.setFont(new Font("宋体", Font.BOLD, 12));
		btn_send.setBackground(new Color(178, 143, 206));
		...
		northPanel.setBackground(new Color(240, 255, 255));
		...
		leftScroll.setBackground(new Color(240, 255, 255));
		rightScroll.setBackground(new Color(240, 255, 255));
		southPanel.setBackground(new Color(240, 255, 255));
		...
		txt_name.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getButton() == MouseEvent.BUTTON1) {
					if ("txt_name".equals(name)) {
						JTextField txt_name = (JTextField) component;
						if (TIP.equals(txt_name.getText())) {
							txt_name.setText("");
						}
					} else if ("panel".equals(name)) {
						JTextField txt_name = (JTextField) ((JPanel) component).getComponents()[0];
						if ("".equals(txt_name.getText())) {
							txt_name.setText(TIP);
						}
					}
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					try {
						...
						throw new Exception("端口号应为1024~65535的整数！");
					}

...
public class Server {
	...
	private JTextField txt_message;// 用于编辑显示文本信息
	...
	private JScrollPane leftPanel;// 左边滚动条
	private JScrollPane rightPanel;// 右边滚动条
	private JSplitPane centerSplit;// 分割线
	private JList<String> userList;// 用户列表组件
	private DefaultListModel<String> listModel;// 默认的列表

	private ServerSocket serverSocket;
	private ServerThread serverThread;// 服务器线程
	private ArrayList<ClientThread> clients;// 用户列表

	private boolean isStart = false;// 用来记录服务器启停
	...
	public void closeServer() {
		try {
			...
			for (int i = clients.size() - 1; i >= 0; i--) {
				// 给所有在线用户发送关闭命令
				clients.get(i).getWriter().println("服务器关闭");
